...
private:
	size_t n_nodes;
	void update_nnodes(size_t node_idx) { if (n_nodes < node_idx + 1) n_nodes = node_idx; };
public:
	...
	BasicGraph();
	BasicGraph(size_t, size_t , size_t ); //set the n_nodes and n_edges maximum capcity, this cap could be excceded but this will be slow
	void reset(); 
	void add_nlink(size_t, size_t, nweightType, nweightType);
	void add_tlink(size_t, tweightType, tweightType);
	void get_tlink(size_t, tweightType&, tweightType&);
	void get_nlink(size_t, size_t &, size_t&, nweightType &, nweightType &);
	...
	size_t getNumberOfNodes() { return this->n_nodes; }
...
void BasicGraph<nweightType, tweightType>::saveToMaxflowFile(std::string output_fname)
{
	...
	for (size_t link_id = 0; link_id < t_links.n_tlinks; ++link_id)
	{
	...
	for (size_t offset = 0; offset < (n_links.n_nlinks * 2); offset += 2)
	{
...
template <class nweightType, class tweightType>
BasicGraph<nweightType, tweightType>::BasicGraph(size_t n_nodes, size_t n_tlinks_maxcap, size_t n_nlinks_maxcap)
{
	this->n_nodes = n_nodes;
	this->t_links.n_tlinks = 0;
	this->t_links.maxcap = n_tlinks_maxcap;
	this->t_links.weights = new tweightType[this->t_links.maxcap*2];
	this->t_links.node_ids = new size_t[this->t_links.maxcap];

	this->n_links.n_nlinks = 0;
	this->n_links.maxcap = n_nlinks_maxcap;
	this->n_links.weights = new nweightType[this->n_links.maxcap * 2];
	this->n_links.edge_nodes = new size_t[this->n_links.maxcap * 2];
};
template <class nweightType, class tweightType>
double BasicGraph<nweightType, tweightType>::getAllocatedMemSizeInGB( void )
{
	double size_inbytes = sizeof(tweightType) *this->t_links.maxcap * 2 + sizeof(size_t)*this->t_links.maxcap + sizeof(nweightType) * this->n_links.maxcap * 2 + sizeof(size_t)* this->n_links.maxcap * 2;
...
void BasicGraph<nweightType, tweightType>::reset()
{
	...
	//memset(this->t_links.node_ids, 0, sizeof(size_t)*this->t_links.maxcap);
	//memset(this->n_links.weights, 0, sizeof(nweightType)*this->n_links.maxcap * 2);
	//memset(this->n_links.edge_nodes, 0, sizeof(size_t)*this->n_links.maxcap * 2);
	this->t_links.n_tlinks = 0;
	this->n_links.n_nlinks = 0;
}

template <class nweightType, class tweightType>
void BasicGraph<nweightType, tweightType>::add_nlink(size_t node0_idx, size_t node1_idx, nweightType w_0to1, nweightType w_1to0)
{
	if (this->n_links.n_nlinks == this->n_links.maxcap) //maxcap will be excceded
	{
		...
		size_t * new_edge_nodes = new size_t[n_links.maxcap * 4];
		memcpy(new_edge_nodes, this->n_links.edge_nodes, sizeof(size_t)* this->n_links.n_nlinks * 2);
		memset(new_edge_nodes + this->n_links.n_nlinks * 2, 0, sizeof(size_t)*(n_links.maxcap * 4 - this->n_links.n_nlinks * 2));
		CLNDEL1D(this->n_links.edge_nodes);
		this->n_links.edge_nodes = new_edge_nodes;
		n_links.maxcap *= 2;
	}
	size_t edge_id = this->n_links.n_nlinks * 2;
...
template <class nweightType, class tweightType>
void BasicGraph<nweightType, tweightType>::add_tlink(size_t node_idx, tweightType w_src2node, tweightType w_node2sink)
{
	if (this->t_links.maxcap == this->t_links.n_tlinks) //maxcap will be excceded
	{
		...
		size_t * new_node_ids = new size_t[this->t_links.maxcap * 2];
		memcpy(new_node_ids, this->t_links.node_ids, sizeof(size_t)* this->t_links.maxcap);
		memset(new_node_ids + this->t_links.maxcap, 0, sizeof(size_t)*(t_links.maxcap * 2 - this->t_links.maxcap));
		CLNDEL1D(this->t_links.node_ids);
...
template <class nweightType, class tweightType>
void  BasicGraph<nweightType, tweightType>::get_tlink(size_t link_id, tweightType& w_src2node, tweightType&w_node2sink)
{
...
template <class nweightType, class tweightType>
void BasicGraph<nweightType, tweightType>::get_nlink(size_t edge_idx, size_t & node0_idx, size_t& node1_idx, nweightType& w_0to1, nweightType& w_1to0)
{
	...
	size_t lcl_edge_idx = edge_idx * 2;
...
void BasicGraph<nweightType, tweightType>::saveToFile(std::string fname)
{
	...
	dim2[0] = 1;
	Utils::ArrayInOut<size_t> nodes(this->t_links.node_ids, dim2, 2);
	...
	Utils::ArrayInOut<size_t> edges(this->n_links.edge_nodes, dim2, 2);
...
void BasicGraph<nweightType, tweightType>::saveToFile_Binary(std::string fname)
{
	...
	output_file2.write(reinterpret_cast<const char*>(this->t_links.node_ids), sizeof(size_t)*this->t_links.n_tlinks);
	output_file2.close();
	...
	output_file4.write(reinterpret_cast<const char*>(this->n_links.edge_nodes), sizeof(size_t)* this->n_links.n_nlinks * 2);
	output_file4.close();
